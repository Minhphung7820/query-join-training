...
class CourseRepository
{
  ...
  private function getMyCourses($request)
  {
    $query = DB::table('courses')
      ->join('course_invoices', 'courses.id', '=', 'course_invoices.course_id')
      ->join('invoices', 'course_invoices.id', '=', 'invoices.course_invoice_id')
      ->join('course_invoice_schedules', 'course_invoice_schedules.course_id', '=', 'course_invoices.id')
      ->join('users as lecturer', 'lecturer.id', '=', 'course_invoices.lecturer_id')
      ->where('invoices.status', 'approved')
      ->where('invoices.invoice_type', 'invoice_course')
      ->whereRaw('course_invoices.id = (
        SELECT MAX(ci.id)
      ...
      ->whereRaw('invoices.id = (
        SELECT MAX(i.id)
        FROM invoices i
        WHERE i.course_invoice_id = course_invoices.id
    )')
      ->when(isset($request['caught_by_user_id']), function ($query) use ($request) {
        return $query->where(function ($query) use ($request) {
          $query->where('invoices.status', 'approved');
          ...
          return $query->when($request['classification_course_app_student'] == 'offline', function ($query) use ($request) {
            return $query->where('courses.classification', 'offline')
              ->when(
                isset($request['status_learning']),
            ...
            return $query->where('courses.classification', 'online')
              ->when(
                isset($request['status_learning']),
                function ($query) use ($request) {
                  return $query->where(function ($query) use ($request) {
  ...
  public function find($id)
  {
    $data = Course::with([
      ...
      'lecturer:id,full_name,avatar,phone,email',
      'category:id,name'
    ])->withCount('ratings')->withTrashed()->findOrFail($id);
